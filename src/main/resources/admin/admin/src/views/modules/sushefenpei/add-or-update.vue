	...
	<div class="addEdit-block" :style='{"padding":"30px 0 0 0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="床位号" prop="chuangweihao">
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/xuesheng/xueshengxuehao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.xueshengxuehaoOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
			...
			xueshengxuehaoChange () {
				this.$http({
					url: `follow/xuesheng/xueshengxuehao?columnValue=`+ this.ruleForm.xueshengxuehao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
